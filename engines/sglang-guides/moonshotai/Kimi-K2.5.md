...
Kimi-K2.5 is Moonshot AI's trillion-parameter native multimodal agentic MoE model
(approximately 32B active parameters per token) with native INT4 weights, combining
vision-language understanding with tool calling and thinking modes. This guide covers serving
it with **SGLang**. The checkpoint served is `moonshotai/Kimi-K2.5` (native INT4). As a
multimodal model, it accepts OpenAI-compatible image inputs via the `image_url` content type
in chat messages.
...
docker run --gpus all --ipc=host -p 30000:30000 \
  ...
  python3 -m sglang.launch_server --model-path moonshotai/Kimi-K2.5 \
