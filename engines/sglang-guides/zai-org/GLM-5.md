...
GLM-5 is a 744B-parameter frontier MoE model from Z-AI with 40B active
parameters and a 202K-token context window. This guide covers serving it with
**SGLang**. Both BF16 (`zai-org/GLM-5`) and native FP8 (`zai-org/GLM-5-FP8`)
checkpoints are published; FP8 fits on a single 8×H200 node, BF16 requires
...
docker run --gpus all --ipc=host -p 30000:30000 \
  ...
  python3 -m sglang.launch_server --model-path zai-org/GLM-5-FP8 \
...
python3 -m sglang.launch_server --model-path zai-org/GLM-5 \
  --trust-remote-code \
  --tp 32 \
