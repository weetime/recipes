...
- `engines/sglang/<org>/<repo>.yaml` — **SGLang engine blocks** (generated by `scripts/sync-sglang.mjs` from the vendored `upstream/sglang/` snapshot — `scripts/fetch-sglang-upstream.mjs` pins the upstream commit in `upstream/sglang.lock`; pure mapping lives in `src/lib/engines/sglang-transform.js`. Only models that also have a vLLM recipe get a block; per-hardware tp is carried in `tp_by_hardware`. Run both via `pnpm sync:sglang`, then regenerate the API golden). `src/lib/engines/sglang/index.js` renders `python3 -m sglang.launch_server …` from a block; `src/lib/engines/sglang-join.js` attaches `recipe.engines = { vllm:{min_version}, sglang:<block> }` + `default_engine` when a block exists (no-op otherwise). Block resolution is **generated-wins**: it reads `engines/sglang/<hf_id>.yaml` first, then falls back to a hand-authored `engines/sglang-manual/<hf_id>.yaml` — the escape hatch for models the upstream cookbook doesn't cover yet (`sync-sglang.mjs` never wipes that tree, so manual blocks survive re-syncs; delete the manual file once a generated block appears). See `engines/sglang-manual/README.md`. `engines.vllm` is a descriptor, NOT the recipe (CommandBuilder serializes `recipe` for client hydration — a self-reference would crash). The Engine pill row in `CommandBuilder.jsx` shows only when `recipe.engines` has 2+ entries; `src/lib/engine-ui.js` (`engineSources`) drives engine-aware variant/strategy/feature options.
